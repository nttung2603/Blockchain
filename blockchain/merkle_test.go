...
func TestNewMerkleNode(t *testing.T) {
	...
	assert.Equal(t, root, fmt.Sprintf("%x", tree.RootNode.Hash), "Merkle node root has is equal")
...
func TestMerklePath(t *testing.T) {
	...
	tree := NewMerkleTree(data)
	path, indices := tree.GetMerklePath([]byte("node4"))
	fmt.Printf("%x\n", path)
	fmt.Printf("%x\n", indices)
	assert.Equal(t, 1, 1, "Merkle node root has is equal")
}
